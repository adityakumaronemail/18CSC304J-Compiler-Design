...
def IndirectTriple(pos):
        ...
        for i in pos:
          ...
          if i not in OPERATORS:
            ...
            c=c+1
            m=m+1
            print("{0:^4s} | {1:^4s} | {2:^4s}     {3:^4d} | ({4:^4d})".format(i,op1,"(-)",m,c))
            x = x+1
            if stack != []:
              op2 = stack.pop()
              op1 = stack.pop()
              c=c+1
              m=m+1
              print("{0:^4s} | {1:^4s} | {2:^4s}     {3:^4d} | ({4:^4d})".format("+",op1,op2,m,c))
              stack.append("(%s)" %x)
              x = x+1
          elif i == '=':
            op2 = stack.pop()
            ...
            c=c+1
            m=m+1
            print("{0:^4s} | {1:^4s} | {2:^4s}     {3:^4d} | ({4:^4d})".format(i,op1,op2),m,c)
          else:
            op1 = stack.pop()
            if stack != []:           
              op2 = stack.pop()
              c=c+1
              m=m+1
              print("{0:^4s} | {1:^4s} | {2:^4s}     {3:^4d} | ({4:^4d})".format(i,op2,op1,m,c))
              stack.append("(%s)" %x)
              x = x+1
print("\nThe indirect triple for given expression:\n")
print("  OP | ARG 1 |ARG 2  MEMORY | STATEMENT ")
IndirectTriple(pos)
